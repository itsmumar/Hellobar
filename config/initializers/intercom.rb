IntercomRails.config do |config|
  ...
  config.user.custom_data = {
    ...
    total_views: proc { |user| user.sites.map { |site| site.bar_statistics.values.sum(&:views) }.sum },
    total_conversions: proc { |user| user.sites.map { |site| site.bar_statistics.values.sum(&:conversions) }.sum }
  }
