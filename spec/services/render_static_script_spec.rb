    ...
    context 'when options[:compress]' do
      ...
      it 'returns compressed script' do
        ...
        allow(StaticScriptAssets)
          .to receive(:render_compressed).with('modules.js', site_id: site.id).and_return 'compressed_modules.js'
        allow(StaticScriptAssets)
          .to receive(:render_compressed).with('static_script_template.js', site_id: site.id).and_return '$INJECT_MODULES; $INJECT_DATA'
