  ...
  class DataAPIHelper
    module RequestParts
      class << self
        ...
        # at a time. Also there is an option limit (without this all contacts
        # will be returned). However, note there is no offset option so
        # pagination is not possible. The limit will return the X newest
        # contacts. In the additional params it is possible specify {"f"=>"c"}
        ...
        #
        def get_contacts(site_id, contact_list_id, read_key, limit=nil, additional_params={})
          return *sign_path_and_params(generate_path("e", site_id, contact_list_id), {"l"=>limit.to_i}.merge(additional_params), read_key)
        end
