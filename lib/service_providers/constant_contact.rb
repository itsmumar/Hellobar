class ServiceProviders::ConstantContact < ServiceProvider
  ...
  def subscribe(list_id, email, name = nil, list = nil)
    ...
    add_contact(cc_contact)
  rescue RestClient::Conflict
    cc_contact = @client.get_contact_by_email(@token, email).results[0]
    cc_contact.lists ||= []
    cc_contact.lists << cc_list

    update_contact(cc_contact)
  end

  def update_contact(contact)
    @client.update_contact(@token, contact, !!@site.try(:double_opt_in))
  rescue RestClient::Conflict
  ...
  def add_contact(contact)
    @client.add_contact(@token, contact, !!@site.try(:double_opt_in))
  rescue RestClient::BadRequest => e
  ...
  def batch_subscribe(list_id, subscribers)
    ...
    subscribers.each do |subscriber|
      subscribe(list_id, subscriber[:email], subscriber[:name], cc_list)
    end
  end
end
